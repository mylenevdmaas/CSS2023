...
def conn_matrix_not_so_basic(n, fraction_of_zeros):
    """Returns nxn symmatric matrix for J with random numbers in [0,1]."""
    J_tri = np.tril(np.random.uniform(1, 1, size=(n, n)), -1)
    J = np.zeros((n,n)) + J_tri + J_tri.T
...
#@njit
def multi_metropolis(n_simulations, n_iterations, T, n, fraction_zeros):
    ...
    return mean_magnet, std_magnet, mean_sus, std_sus, mean_corr_matrix
    
#@njit
def run_simulation(n_simulations:int, n_iterations:int, T_list:np.array, n:int, fraction_zeros):
    ...
    for i, T in enumerate(T_list):
        means_mag[i], stds_mag[i], means_sus[i], stds_sus[i], _ = multi_metropolis(n_simulations, n_iterations, T, n, fraction_zeros)
